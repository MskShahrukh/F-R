...
export const fetchProducts = async (args: fetchProductsArgs) => {
  ...
  https.get(args._url, (res) => {
    const writeLocalCSV = res.pipe(fs.createWriteStream('products.csv'));

    writeLocalCSV.on('finish', () => {
      console.info(`Fetched and wrote a new file products.csv on root...`);
      readProductsFromLocal();
    });
  });

  const readProductsFromLocal = async () => {
